...
export async function runChatGptMcpServer(options: { statePath?: string; herdrClient?: HerdrClient } = {}): Promise<void> {
  ...
  server.registerTool("file_read", {
    ...
    description: "Read text directly or transfer a PNG, JPEG, GIF, or WebP image as native MCP image content. This is the preferred tool for model-side visual inspection of local images; do not use terminal ASCII/image renderers as a substitute when native MCP image transfer is available. Large or high-resolution images are automatically downscaled and encoded as a compact WebP preview without modifying the local file, reducing conversation-context usage. To visibly render an image for the user in the conversation, call file_image_preview with the same path and workspace after inspection. The resolved path and disclosed workspace are checked unless full access is selected. Images default to a 1.5 MB transfer limit and never appear as base64 text.",
  ...
  server.registerTool("file_edit", {
    ...
    description: "Replace exact text in one existing UTF-8 text file. By default old_text must occur exactly once. When expected_occurrences is provided, the file must contain exactly that many non-overlapping matches and all of them are replaced. No fuzzy matching is performed. Existing and resulting files are capped at 16 MiB. Disabled in read-only mode; workspace-write rejects path traversal and symlink escapes.",
