...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  "When codexluna_status completes with image_preview_recommended=true, automatically call file_image_preview exactly once for image_preview_path using the returned workspace_path and permission_mode, even if the user did not explicitly ask to see the image. Before calling, compare image_preview_content_key with prior successful file_image_preview results in this conversation and skip the call when the same image_content_key was already presented. Repeated codexluna_status polling must never create duplicate preview cards. By default auto-present only the single recommended image; present additional image_artifacts only when the user explicitly asks or they are materially necessary to answer. codexluna_status may include native image content for inspection but never mounts image-preview UI itself, and an image is displayed only after file_image_preview succeeds.",
