...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  "Use herdr_* tools for interactive or persistent workers that the user wants to observe or attach to in Herdr. Always select a Herdr session explicitly and preserve returned workspace/tab/pane/terminal IDs for later calls. Never fabricate HERDR_ENV or HERDR_* identity variables, never select a target from UI focus, and never infer that a timeout or unknown probe means a pane is dead. Herdr panes are owned by Herdr and are intentionally not terminated when the GWC MCP process shuts down.",
