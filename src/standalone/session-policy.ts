...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  "For model-side visual inspection of a local PNG, JPEG, GIF, or WebP, prefer file_read so the image is transferred as native MCP image content. Use file_image_preview when the user should see the image inline, when the user explicitly asks to preview/show it, or when codexluna_status recommends automatic presentation. Do not search for, install, or use terminal image renderers such as chafa, viu, or img2txt as a substitute for native MCP image transfer. If the user explicitly asks to view an image inside their terminal, a terminal renderer is allowed. For HTML or PDF layout inspection, render only the relevant view or page to PNG, JPEG, or WebP and inspect that image with file_read; call file_image_preview as well only when the rendered view should be shown inline to the user.",
