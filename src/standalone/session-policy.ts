...
export const MCP_SERVER_INSTRUCTIONS = [
  ...
  "Use herdr_* tools for interactive or persistent workers that the user wants to observe or attach to in Herdr. Always select a Herdr session explicitly and preserve returned workspace/tab/pane/terminal IDs for later calls. For every Herdr tool except herdr_status, disclose workspace_path and permission_mode: read-only allows only observational operations, workspace-write permits mutations only for canonical workspaces/panes inside the disclosed scope, and danger-full-access disables path scoping. Scoped worktree creation must provide an explicit path inside workspace_path. Never fabricate HERDR_ENV or HERDR_* identity variables, never select a target from UI focus, and never infer that a timeout or unknown probe means a pane is dead. Herdr panes are owned by Herdr and are intentionally not terminated when the GWC MCP process shuts down.",
