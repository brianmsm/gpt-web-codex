...
- Every Herdr operation except `herdr_status` carries an explicit `workspace_path` and `permission_mode`. Scoped modes canonicalize existing paths with `realpath`, reject symlink escapes, authorize panes/workspaces against Herdr worktree metadata or pane cwd, and block all Herdr mutations in `read-only`. `danger-full-access` deliberately removes this path-scope restriction.
