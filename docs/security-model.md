...
- Every Herdr operation except `herdr_status` carries an explicit absolute `workspace_path` and `permission_mode`. Relative Herdr path arguments resolve against that disclosed workspace rather than the MCP process cwd. Scoped modes canonicalize existing paths with `realpath`, reject symlink escapes, authorize panes/workspaces against Herdr worktree metadata or pane cwd, and block all Herdr mutations in `read-only`. `danger-full-access` deliberately removes the containment restriction but keeps the same deterministic path base.
- GWC performs a Herdr `ping` compatibility check before every operational request and refuses protocols other than the validated protocol 20 instead of assuming forward compatibility.
- `herdr_pane_run` and `herdr_pane_send` are arbitrary PTY control surfaces and are annotated as destructive/open-world. The Herdr MVP still exposes no generic RPC router and no dedicated lifecycle operation such as workspace/pane close, worktree remove, kill, or stop; timeouts and missing observations are not treated as proof of process death.
